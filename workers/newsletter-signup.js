...
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 580px; margin: 0 auto; background-color: #ffffff; border-radius: 4px; overflow: hidden;">
    ...
    <!-- Header bar -->
    <tr>
        <td style="background-color: #fffafa; padding: 24px 36px; text-align: center;">
            <img src="https://indigopalm.co/email-images/logo-full.png" alt="Indigo Palm Collective" width="160" style="display: inline-block; width: 160px; height: auto;" />
        </td>
    </tr>
    ...
    <tr>
        <td style="padding: 0; line-height: 0;">
            <img src="https://indigopalm.co/email-images/terra-luz-hero.jpg" alt="Terra Luz backyard with putting green and covered patio, Indio CA" width="580" style="display: block; width: 100%; max-width: 580px; height: 260px; object-fit: cover;" />
        </td>
    </tr>
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      console.error('Newsletter signup error:', error.message, error.stack);
      return new Response(JSON.stringify({
        success: false,
...
async function sendWelcomeEmail(email, env) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Resend welcome failed: ${err}`);
  }
}
...
async function sendOwnerNotification(email, env) {
  ...
  if (!response.ok) {
    const err = await response.text();
    throw new Error(`Resend owner notification failed: ${err}`);
  }
}
