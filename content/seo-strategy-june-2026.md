...
**Prepared:** June 18, 2026 | Multi-agent audit (Google Search Console + Pinterest + Business Metrics)

---

# PART 1: CURRENT STATE SNAPSHOT

**Organic Search.** The domain is 4 months old and behaving exactly as expected for its age — not good, not broken, just early. Google has indexed 10 pages, the site has crossed 50 clicks in its first 28 days of measurable traffic, and the palm-springs-vs-indio post is the only real traffic engine right now (252 impressions in one week, April 12-18). The structural SEO work is genuinely solid — schema markup, canonical tags, FAQPage structured data on 48 posts — this is better than 95% of boutique operators. The active damage is specific and fixable: 8 posts accidentally noindexed, 4 sitemap URLs returning 404s, API endpoints being crawled instead of blocked, and a redirect validation failure affecting 13 pages. These aren't strategy problems, they're execution errors, and they're suppressing pages that may already have earned ranking signals. The content moat (74+ posts) is real but underpowered by inbound links and topical authority gaps vs. cactushugs.com, the most direct local competitor.

**Pinterest.** This channel is in its infancy and underperforming for one main reason: the content pipeline exists but isn't being published reliably. 723 impressions in 31 days, 5 outbound clicks total — the account hasn't crossed Pinterest's algorithm trust threshold yet. The one bright spot: the Cozy Cactus board converts at 4x the rate of the editorial Blogs board (outbound clicks, saves). The 355 pins written for 71 blog posts were delivered to Sabbir on May 25 with no confirmed posting. The 24 dry-run pins from May 16 were never published. The Sundune has zero Pinterest presence. This is a contractor accountability problem masking a distribution problem — the content exists, the boards exist, it's just not going up. Until posting cadence hits 4-5 pins/day consistently, Pinterest will not generate meaningful referral traffic.

**Direct Bookings.** Five percent of revenue comes from direct channels today — zero confirmed truly-direct bookings in the Gmail data window. All 12 confirmed bookings in the period were Airbnb-sourced. The financial case for shifting this is clear: a single Coachella weekend booking at $1,800/night generates ~$540 in guest-side Airbnb fees alone. Capture 3 festival bookings direct and you've put ~$1,600 back in guests' pockets (or your margin). The website exists, it has property pages, it has a booking widget — but there is no dedicated "book direct" landing page, no fee comparison, no explicit value proposition for why a guest should bypass Airbnb. The funnel has no top-of-funnel hook and no bottom-of-funnel close for direct intent traffic.
...
2. The 355 AI-written pins for 71 blog posts were delivered May 25. No confirmed posting. This is a Sabbir accountability issue.
...
**Gap 1: `/blog/coachella-2027-where-to-stay/`** — No post exists for this query. Highest-revenue content gap. Publish by August 2026. Target 2,500-3,000 words.

**Gap 2: `/blog/bnp-paribas-open-vacation-rental-guide/`** — Open lane. Nobody with vacation rental inventory is in this SERP. Publish by September 2026. Target 2,000+ words.

**Gap 3: `/vacation-rentals/indio/`** — Geo-targeted landing page. VARE Properties ranks here and they're smaller than Indigo Palm. 2-hour build.

**Gap 4: `/book-direct/`** — Standalone page making the explicit case for booking direct. Fee comparison table, trust signals, FAQ ("is it safe to book direct?"). 1-week build.

**Gap 5: `/vacation-rentals/palm-springs/`** — Geo page for Palm Springs. Longer-term (6-12 months to rank competitively) but should exist now to start accumulating crawl history.

**Gap 6: `/vacation-rentals-with-private-pool/`** — Every Indigo Palm property has a private pool. Build the amenity landing page.

**Gap 7: BNP Paribas existing post update** — `/blog/bnp-paribas-indian-wells-where-to-stay/` is in the "crawled but not indexed" list. Add "2027" to H1 and title tag. Update tournament dates. 30 minutes of work.
...
**1. 8 posts accidentally noindexed** (30 min)
Google Search Console → Coverage → Excluded → "Excluded by noindex tag." Find each post's markdown file in `/content/blog/` and remove `noindex: true` or `robots: noindex` from frontmatter. Request reindexing in GSC for all 8 URLs.

Posts affected:
- `/blog/cozy-cactus-origin-story/`
- `/festival-guide-cozy-cactus/`
- `/blog/palm-springs-coffee-guide/`
- Plus 5 others in the GSC "Excluded by noindex" list

**2. API endpoints in the crawl report** (10 min)
Add to robots.txt immediately:
```
Disallow: /api/
Disallow: /api/availability
```

**3. 4 sitemap URLs returning 404** (5 min)
Run `npm run build` — resolves sitemap 404s AND publishes the dog-friendly and bachelorette posts.
...
1. Fix noindex on 8 blog posts (30 min, unlocks indexed pages that may already have authority)
2. Block /api/ in robots.txt (10 min, stops crawl budget waste today)
3. Run `npm run build` — publishes dog-friendly and bachelorette posts, resolves 4 sitemap 404s (5 min)
4. Add "book direct" fee comparison block to all 4 property pages (2 hours, direct revenue impact)
5. Add Indian Wells proximity copy to Terra Luz and Cozy Cactus property pages (30 min, captures BNP query cluster immediately)
6. Add UTM parameters to all Pinterest pin destination URLs (1 hour, enables attribution for the first time)
7. Confirm with Sabbir within 5 business days: which of the 355 pins are live, current weekly posting count, May analytics spreadsheet. If no clear answer, bring posting in-house.
8. Publish Coachella 2027 post before August (highest single-post revenue impact in the portfolio)
9. Publish BNP Paribas vacation rental guide before September (open lane, no competition)
10. Build `/vacation-rentals/indio/` geo landing page (2 hours, targets year-round commercial intent)
...
**TASK 10: Sabbir Accountability Checkpoint (30 min, Unblocks Pinterest)**
Email Sabbir asking for exactly three things:
1. Which of the 355 pins (delivered May 25) are currently live — screenshot or spreadsheet
2. Current weekly posting count (target: 4-5 pins/day)
3. May analytics export from Pinterest (impressions, saves, outbound clicks by board)

If no clear answers within 5 business days: bring posting in-house using Tailwind. At 5 pins/day, 355 pins = 71 days of content. Posting takes 20 minutes/day in Tailwind once pins are written — and they are.

---

## WEEK 3-6: CONTENT SPRINTS

**POST 1: Coachella 2027 Where to Stay**
- URL: `/blog/coachella-2027-where-to-stay/`
- Publish by: August 2026
- Target: 2,500-3,000 words
- Structure: H1 "Where to Stay for Coachella 2027: The Complete Guide to Festival Rentals Near the Stage," Indio vs. Palm Springs section, private rental vs. hotel cost comparison, Cozy Cactus + Terra Luz profiles, festival logistics, FAQPage schema, booking CTA
- Links to: both Indio property pages, palm-springs-vs-indio post

**POST 2: BNP Paribas Open Vacation Rental Guide**
- URL: `/blog/bnp-paribas-open-vacation-rental-guide/`
- Publish by: September 2026
- Target: 2,000-2,500 words
- Structure: H1 "Where to Stay for the BNP Paribas Open 2027: Private Vacation Rentals Near Indian Wells," geography section, private rental vs. hotel comparison, Terra Luz + Cozy Cactus profiles, booking timeline ("book in October-November"), FAQPage schema
- Why this is the highest-upside bet: see THE BIG BET section below

**POST 3: Stagecoach 2027 Expansion**
- Expand existing `/blog/stagecoach-2027-where-to-stay/` from ~1,800 to 2,500+ words
- Publish by: October 2026
- Add: festival logistics section, Coachella vs. Stagecoach vibe comparison, 2027 FAQPage schema, expanded property sections
- After expanding: GSC URL Inspection → Request Indexing

**POST 4: Vacation Rentals Indio CA — Geo Landing Page**
- URL: `/vacation-rentals/indio/`
- Publish by: Week 4
- Target: 400-600 words (landing page, not blog post)
- Structure: H1 "Vacation Rentals in Indio, CA — Book Direct," opening paragraph on Coachella Valley location, Cozy Cactus + Terra Luz property cards, fee comparison, internal links to festival guides

**POST 5: Book Direct Landing Page**
- URL: `/book-direct/`
- Publish by: Week 3
- Target: 300-500 words
- Structure: H1 "Skip Airbnb Fees — Book Your Coachella Valley Vacation Rental Direct," fee comparison table (show real numbers), what you get by booking direct, trust signals (Superhost badge, review count), FAQ, links to each property with availability
